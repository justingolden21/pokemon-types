window.addEventListener('load', () => {
	...
	document
		.querySelector('a[href="#search-string-modal"]')
		.addEventListener('click', () => {
			let typeHTML = '';
			if (currentType1)
				typeHTML += `${getTypeIcon(currentType1)} ${capitalize(
					currentType1
				)}`;
			if (currentType1 && currentType2) typeHTML += ' &nbsp; ';
			if (currentType2)
				typeHTML += `${getTypeIcon(currentType2)} ${capitalize(
					currentType2
				)}`;
			document.getElementById('search-string-type-display').innerHTML =
				typeHTML;
			updateSearchString();
		});
	...
	document
		.getElementById('has-charge-move-toggle')
		.addEventListener('change', () => {
	...
	document
		.getElementById('super-against-toggle')
		.addEventListener('change', () => {
			document.getElementById('multiplier-threshold-div').style.display =
				document.getElementById('super-against-toggle').checked
					? 'block'
					: 'none';
		});
...
function updateSearchString() {
	...
	const superAgainst = document.getElementById(
	...
	if (!superAgainst) {
		if (currentType1 == '' || currentType2 == '') {
			...
			if (hasFast && hasCharge) str += '&';
			if (hasCharge)
				str += `@2${currentType1},@2${currentType2},@3${currentType1},@3${currentType2}`;
		}
	} else {
		let matchups = [];

		let minMultiplier = parseFloat(
			selectVal('multiplier-threshold-options')
		);
		...
		for (let type of TYPE_NAMES) {
			...
			if (usingWeatherBoost && isBoosted([type, ''], currentWeather))
				matchup *= 1.2;
